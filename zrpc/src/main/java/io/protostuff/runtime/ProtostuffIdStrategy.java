 ...
 * @date 2017/9/14
 */
public final class ProtostuffIdStrategy extends IdStrategy
{
    ...
    static final ConcurrentHashMap<String,Class> CLASS_MAP=new ConcurrentHashMap<>();
    
    
    public ProtostuffIdStrategy()
    {
        super(DEFAULT_FLAGS, null, 0);
    }

    public ProtostuffIdStrategy(IdStrategy primaryGroup, int groupId)
    {
        super(DEFAULT_FLAGS, primaryGroup, groupId);
    }

    public ProtostuffIdStrategy(int flags, IdStrategy primaryGroup, int groupId)
    {
    ...
    {
        return CLASS_MAP.computeIfAbsent(className, key->{
            try {
    ...
    private EnumIO<? extends Enum<?>> getEnumIO(String className, boolean load)
    {
        ...
        if (eio == null)
        {
            ...
            final Class<?> enumClass = loadClass(className);
            ...
            throws IOException
    {
        ...
        if (factory == null)
        {
            ...
            {
                factory = new RuntimeMapFactory(loadClass(className));
                MapSchema.MessageFactory f = mapMapping.putIfAbsent(className,
                        factory);
                if (f != null)
                    factory = f;
